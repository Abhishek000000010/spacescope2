...
import { 
  ...
  Info, Zap, Cpu, ListChecks, ArrowRight
} from 'lucide-react';
...
const topics = [
  { 
    id: 't1', 
    title: 'Black Holes', 
    desc: 'Gravitational monsters of the universe.', 
  ...
  { 
    id: 't2', 
    title: 'Space Weather', 
    desc: 'Understanding the Sun-Earth connection.', 
    color: 'from-orange-600/20 to-black', 
    img: 'https://images.unsplash.com/photo-1534067783941-51c9c23ecefd?auto=format&fit=crop&q=80&w=1200' 
  },
  { 
    id: 't3', 
    title: 'Exoplanets', 
    desc: 'Searching for Earth 2.0 beyond our solar system.', 
    color: 'from-cyan-600/20 to-black', 
    img: 'https://images.unsplash.com/photo-1614314107768-6018061b5b72?auto=format&fit=crop&q=80&w=1200' 
  },
  { 
    id: 't4', 
    title: 'The ISS', 
    desc: 'Humanity\'s outpost in low Earth orbit.', 
...
const Learn: React.FC = () => {
  ...
  const generateAIQuiz = async (topicTitle?: string) => {
    ...
    try {
      ...
      setQuestions(fallbackQuiz); // Fallback if AI fails
    } finally {
              ...
              <div className="p-8 overflow-y-auto no-scrollbar flex-1 bg-[#030014]/50">
                <form onSubmit={handleAISearch} className="relative mb-8">
                  ...
                  <button 
                    ...
                    className="absolute right-4 top-1/2 -translate-y-1/2 p-3 bg-cyan-600 hover:bg-cyan-500 rounded-xl transition-all disabled:opacity-50 shadow-lg"
                  >
                ...
                {searchResult && !isSearching && (
                  ...
                  >
                    <div className="p-8 rounded-[2rem] bg-gradient-to-br from-cyan-600/10 to-transparent border border-cyan-500/20">
                        ...
                        {searchResult.sources.length > 0 && (
                          <div className="p-8 rounded-3xl border border-white/5">
                            <h4 className="text-xs font-bold text-white/40 uppercase tracking-[0.2em] mb-6 flex items-center gap-2">
                              <Globe size={16} /> Grounded Sources
                            </h4>
                            <div className="space-y-2">
                              {searchResult.sources.map((source, idx) => (
                                <a 
                                  ...
                                  className="p-3 bg-white/5 border border-white/5 rounded-xl hover:border-cyan-500/30 hover:bg-white/10 transition-all flex items-center justify-between group"
                                >
                                  <span className="text-xs text-gray-500 truncate pr-4 group-hover:text-cyan-400 transition-colors">
                                    {source.title}
                                  </span>
           ...
           {isGeneratingQuiz ? (
             ...
             <div className="glass-card p-12 rounded-[3rem] text-center">
               ...
               <div className="flex gap-4">
                 <button 
                   onClick={() => generateAIQuiz(activeTopic ? topics.find(t => t.id === activeTopic)?.title : undefined)}
                   className="flex-1 py-4 bg-purple-600 hover:bg-purple-500 rounded-2xl font-bold transition-all flex items-center justify-center gap-2"
                 >
                   <RefreshCw size={20} /> New AI Quiz
                 </button>
                 <button 
                   onClick={backToHub}
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
           ...
           <div className="max-w-4xl mx-auto">
             ...
             <div className="space-y-6 text-gray-300 text-lg leading-relaxed">
               <p>
                 Deep dive into the fascinating world of {topics.find(t => t.id === activeTopic)?.title.toLowerCase()}. This domain represents one of the most critical areas of modern astrophysics, combining quantum mechanics with general relativity.
               </p>
               <div className="bg-white/5 p-8 rounded-2xl border-l-4 border-cyan-500">
                 ...
                 <p className="text-sm italic">
                   {activeTopic === 't1' 
                     ...
                     : "Thousands of exoplanets have been discovered, some of which exist within the 'Goldilocks zone' where liquid water could exist."}
                 </p>
               </div>
               <p>
                 Researchers are constantly using new tools like the James Webb Space Telescope to uncover secrets that were previously hidden by cosmic dust and immense distances.
               </p>
             </div>
             <div className="mt-16 pt-8 border-t border-white/10 flex justify-between items-center">
               <button 
                onClick={() => generateAIQuiz(topics.find(t => t.id === activeTopic)?.title)} 
                className="px-8 py-3 bg-cyan-600 hover:bg-cyan-500 text-white rounded-xl transition-all font-bold shadow-lg shadow-cyan-900/20"
               >
                 Take Topic Quiz
               </button>
               <button onClick={() => setActiveTopic(null)} className="text-cyan-400 font-bold hover:underline">
                 Next Topic &rarr;
               </button>
             </div>
           </div>
        ...
        <>
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-6">
            ...
            <button 
              ...
              className="px-6 py-3 bg-cyan-600/10 hover:bg-cyan-600/20 border border-cyan-500/30 text-cyan-400 rounded-xl font-bold flex items-center gap-2 transition-all shadow-lg shadow-cyan-900/10 group"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
            {topics.map(topic => (
              <motion.div
                ...
                className={`relative group h-80 rounded-3xl overflow-hidden border border-white/10 cursor-pointer`}
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 p-8">
                  ...
                  <div className="mt-6 flex items-center gap-2 text-cyan-400 font-bold uppercase text-xs tracking-widest">
          ...
          <div className="glass-card p-12 rounded-[3rem] text-center border-purple-500/30 relative overflow-hidden">
            ...
            <p className="text-gray-400 max-w-xl mx-auto mb-10 text-lg">
              Take our weekly mini-quiz and earn Space Scope achievement badges. 
              Top scorers get exclusive access to premium missions.
            </p>
            <button 
              onClick={() => generateAIQuiz()}
              className="px-12 py-4 bg-purple-600 hover:bg-purple-500 rounded-2xl font-bold transition-all shadow-xl shadow-purple-900/40 text-lg"
            >
              Start AI Mini Quiz
            </button>
          </div>
