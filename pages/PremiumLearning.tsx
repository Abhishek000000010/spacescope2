...
const PremiumLearning: React.FC = () => {
  ...
  const startLesson = async (course: any, moduleIndex: number) => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: `Generate a professional, high-level mission briefing for the module: "${module.title}" in the course "${course.name}". Deep dive into: ${module.context}. Use highly scientific and futuristic terminology.`,
        config: {
  ...
  const startCertificationExam = async (course: any) => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        model: "gemini-3-flash-preview",
        contents: `Generate 5 expert-level multiple choice questions for a certification exam on the course "${course.name}". Focus on these topics: ${course.modules.map((m: any) => m.title).join(', ')}. Difficulty: Hard.`,
        config: {
          responseMimeType: "application/json",
          responseSchema: {
            type: Type.ARRAY,
  ...
  const handleDownload = async () => {
    ...
    try {
      // Create a high-quality capture of the certificate
      const canvas = await html2canvas(certRef.current, {
        scale: 2, // Double resolution for crisp PDF
        useCORS: true,
        backgroundColor: '#030014', // Match theme background
        logging: false,
      });
  ...
  return (
    <div className="min-h-screen pt-24 pb-12 px-4 max-w-7xl mx-auto">
      <div className="text-center mb-16">
        ...
        <h1 className="text-4xl md:text-5xl font-orbitron font-bold mb-4 neon-text">Advanced Learning Hub</h1>
        ...
        {isPremium && (
          <motion.div 
            ...
            className="mt-6 inline-flex items-center gap-2 px-4 py-2 bg-cyan-500/10 border border-cyan-500/30 rounded-full text-cyan-400 text-xs font-bold uppercase tracking-widest"
          >
            <ShieldCheck size={14} /> Pro Researcher Access Active
          </motion.div>
        )}
      </div>
            ...
            >
              <div className="p-6 sm:p-8 border-b border-white/10 flex justify-between items-center bg-cyan-900/10">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-xl sm:text-2xl font-orbitron font-bold tracking-tight">
                ...
                {isGeneratingLesson || isGeneratingExam ? (
                  ...
                  !examFinished ? (
                      ...
                      <div className="space-y-4">
                        {examQuestions[currentExamQuestion].options.map((opt, idx) => {
                          ...
                          return (
                            ...
                            >
                              <span>{opt}</span>
                              {hasAnswered && idx === examQuestions[currentExamQuestion].answer && <CheckCircle2 size={20} />}
                            </button>
                          );
                        })}
                      </div>
                    ...
                    <div className="max-w-2xl mx-auto py-20 text-center">
                       ...
                       <h2 className="text-4xl font-orbitron font-bold mb-4">{examScore >= 4 ? "Exam Passed!" : "Exam Failed"}</h2>
                       ...
                       {examScore >= 4 ? (
                         ...
                         <div className="flex gap-4">
                           <button onClick={() => startCertificationExam(activeCourse)} className="flex-1 py-4 bg-white/5 border border-white/10 hover:bg-white/10 rounded-2xl font-bold flex items-center justify-center gap-2"><RefreshCw size={18} /> Retry Exam</button>
                  ...
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-12 pb-12">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                      <div className="lg:col-span-2 space-y-8">
                        ...
                        <div className="bg-black/80 rounded-3xl p-8 border border-white/10 font-mono text-sm overflow-hidden text-cyan-400/80 leading-relaxed overflow-x-auto whitespace-pre-wrap">
                           {lessonContent.simulationData}
                        </div>
                      </div>
                      <div className="space-y-8">
                        <div className="glass-card p-8 rounded-3xl border-white/5 bg-white/5">
                           <h4 className="text-sm font-bold text-white/40 uppercase tracking-widest mb-6 flex items-center gap-2"><Cpu size={18} /> Specs</h4>
